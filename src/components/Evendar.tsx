...
import { EventInput, ViewMode } from '@constants/interfaces';
...
const Evendar = () => {
  ...
  return (
    <S.EvendarContainer>
      <HeaderToolbar onViewModeChange={handleViewModeChange} />
      {viewMode === 'month' ? (
        <MonthlyCalendar
          events={toEventModel([
            {
        ...
        <DailyCalendar events={toEventModel([])} />
      )}
    </S.EvendarContainer>
  );
};
